...
This complete model could be used in the excellent [simsem package ](https://cran.r-project.org/package=simsem) to conduct a power analysis or to perform methodological investigations. To do so, we will also need an analogous model with all the parameters set free.
...
Now we can use `simsem::sim` to create many samples based on the `m_complete` model. These samples will be evaluated with the `m_free` model.

Let's make 1000 samples, of *n* = 200 individuals.

```{r simsem}
# Use simsem to make 100 samples of 200 ineach (real analyses would be larger)
library(simsem)
simsem_output <- sim(
  nRep = 1000, 
  model = m_free, 
...
We can view a summary of the fit statistics with the `simsem::summaryFit` function:

```{r viewfit}
# View summary of fit statistics
summaryFit(simsem_output) %>% 
  kable(digits = 2)
```

Given that the `m_free` model is based directly on the the model that generated the data, it is unsurprising that the `m_free` model fits almost perfectly.

We can view a summary of the standardized parameters with the `simsem::summaryParam` function if we set the `std` parameter to `TRUE`:
...
The Power column shows how often each parameter was found to be significantly different from 0. These results suggest that if the `m_complete` model is correct, all its parameters are likely to be found significantly different from 0 when the sample size is 200.
